...
@SuppressLint("DefaultLocale") public class CaldroidFragment extends DialogFragment {
  ...
  public final static String _SELECTED_BACKGROUND_FOR_DATETIME_MAP =
      "_selectedBackgroundForDateTimeMap";
  ...
  public HashMap<String, Object> getCaldroidData() {
    ...
    caldroidData.put(_SELECTED_BACKGROUND_FOR_DATETIME_MAP, selectedBackgroundForDateTimeMap);
   ...
   */
  public void setBackgroundResourceForDates(HashMap<Calendar, Integer> backgroundForDateMap) {
    ...
    for (Calendar date : backgroundForDateMap.keySet()) {
  ...
  /**
   * Set selectedBackgroundForDateMap
   */
  public void setSelectedBackgroundResourceForDates(
      HashMap<Calendar, Integer> selectedBackgroundForDateMap) {
    if (selectedBackgroundForDateMap == null || selectedBackgroundForDateMap.size() == 0) {
      return;
    }

    selectedBackgroundForDateTimeMap.clear();

    for (Calendar date : selectedBackgroundForDateMap.keySet()) {
      Integer resource = selectedBackgroundForDateMap.get(date);
      DateTime dateTime = CalendarHelper.convertDateToDateTime(date);
      selectedBackgroundForDateTimeMap.put(dateTime, resource);
    }
  }
  ...
  public void setBackgroundResourceForDate(int backgroundRes, Calendar date) {
    DateTime dateTime = CalendarHelper.convertDateToDateTime(date);
    backgroundForDateTimeMap.put(dateTime, backgroundRes);
  }

  public void removeBackgroundResourceForDate(Calendar date) {
    DateTime dateTime = CalendarHelper.convertDateToDateTime(date);
    backgroundForDateTimeMap.remove(dateTime);
  }

  public boolean isDateSelected(DateTime date) {
   ...
   */
  public void setTextColorForDates(HashMap<Calendar, Integer> textColorForDateMap) {
    ...
    for (Calendar date : textColorForDateMap.keySet()) {
  ...
  public void setTextColorForDate(int textColorRes, Calendar date) {
    DateTime dateTime = CalendarHelper.convertDateToDateTime(date);
   ...
   */
  public void moveToDate(Calendar date) {
  ...
  public void prevMonth() {
    if (mListener != null) mListener.onPrevMonthClicked();
    dateViewPager.setCurrentItem(pageChangeListener.getCurrentPage() - 1, true);
  }
  ...
  public void nextMonth() {
    if (mListener != null) mListener.onNextMonthClicked();
    dateViewPager.setCurrentItem(pageChangeListener.getCurrentPage() + 1, true);
  }

  public void setOnSwitchMonthListener(Listener listener) {
    mListener = listener;
  }
   ...
   */
  public void setDisableDates(ArrayList<Calendar> disableDateList) {
    ...
    for (Calendar date : disableDateList) {
      DateTime dateTime = CalendarHelper.convertDateToDateTime(date);
  ...
  public void deselectDate(Calendar date) {
    DateTime dateTime = CalendarHelper.convertDateToDateTime(date);
  ...
  public void setSelectedDates(Set<Calendar> dates) {
    selectedDates.clear();
        ...
        @Override public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
          ...
          if (caldroidListener != null) {
            if (!enableClickOnDisabledDates) {
              if ((minDateTime != null && dateTime.lt(minDateTime)) || (maxDateTime != null
            ...
            Calendar date = CalendarHelper.convertDateTimeToDate(dateTime);
  ...
  protected void refreshMonthTitleTextView() {
    ...
    monthTitle = first + monthTitle.substring(1);
  ...
  protected ArrayList<String> getDaysOfWeek() {
    ...
    for (int i = 0; i < 7; i++) {
      Calendar date = CalendarHelper.convertDateTimeToDate(nextDay);
      String dateString = fmt.format(date.getTime());
      if (dateString.length() == 4 && dateString.substring(3).equals(".")) {
  ...
  public class DatePageChangeListener implements OnPageChangeListener {
    ...
    private void changePage(int position) {
      ...
      CaldroidGridAdapter currentAdapter =
          caldroidGridAdapters.get(position % CaldroidFragment.NUMBER_OF_PAGES);
